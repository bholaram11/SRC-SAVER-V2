...
from devgagan import app, telethon_client as gf
...
class SmartTelegramBot:
    ...
    async def _process_message(self, userbot, msg: Message, sender: int, edit_msg: Message):
        ...
        try:
            ...
            # Adaptive Downloader - parallel workers for faster download
            # workers parameter uses Telegram's internal CDN chunking (safe, no ban risk)
            if file_size > 10 * 1024**2:  # > 10MB
                workers = 3
            elif file_size > 5 * 1024**2:  # > 5MB
                workers = 2
            else:
                workers = 1

            logger.info(
                f"📥 Download START: {filename} | Size: {file_size / (1024**2):.1f}MB | "
                f"Workers: {workers} | MsgID: {msg.id}"
            )

            # FloodWait prevention - pre-emptive rate limiting
            ...
            # Download complete - log final stats
            ...
            logger.info(
                ...
                f"Size: {actual_size / (1024**2):.1f}MB | "
                ...
                try:
                    ...
                    try:
                        ...
                        if upload_method == "Telethon" and gf:
                            ...
                            if edit_msg:
                                await edit_msg.edit("**⚡ Uploading (Telethon)...**")
                            progress_message = await gf.send_message(sender, "**__⚡ Uploading...__**")
                            html_caption = await self.caption_formatter.markdown_to_html(caption)

                            logger.info(f"📤 Upload START: {os.path.basename(file_path)} | Size: {actual_size / (1024**2):.1f}MB | Method: Telethon")
                            ...
                            logger.info(
                                f"✅ Upload COMPLETE: {os.path.basename(file_path)} | "
                                f"Time: {upload_time:.1f}s | Speed: {upload_speed / (1024**2):.1f}MB/s"
                            )
                            ...
                            if edit_msg:
                                await edit_msg.edit("**📤 Uploading (Pyrogram)...**")

                            logger.info(f"📤 Upload START: {os.path.basename(file_path)} | Size: {actual_size / (1024**2):.1f}MB | Method: Pyrogram")
    ...
    async def _copy_public_message(self, app_client, userbot, sender: int, chat_id: str, message_id: int, edit_id: int):
        ...
        try:
            ...
            if userbot:
                ...
                filename, file_size, _ = self.media_processor.get_media_info(msg)
                workers = 3 if file_size > 10 * 1024**2 else 2 if file_size > 5 * 1024**2 else 1
                await flood_preventor.before_request()
                file_path = await userbot.download_media(msg, progress=progress_bar, progress_args=progress_args, workers=workers)
                flood_preventor.after_request()
                file_path = await self.file_ops.process_filename(file_path, sender)
